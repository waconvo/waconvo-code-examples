...
async function main() {
  try {
    ...
    const hotelsList = await waconvo.referenceData.locations.hotels.byCity.get({
      cityCode: "LON",
    });

    // 2. Hotel Search API to get the price and offer id
    const pricingResponse = await waconvo.shopping.hotelOffersSearch.get({
    ...
    const response = await waconvo.booking.hotelBookings.post({
      data: {
